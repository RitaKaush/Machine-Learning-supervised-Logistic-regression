...
costfuncJ = (-1/m) * sum( y .* log(hypothesis) + (1 - y) .* log(1 - hypothesis) );

costRegularizationTerm = lambda/(2*m) * sum( theta(2:end).^2 );

J = costfuncJ + costRegularizationTerm;
...
gradRegularizationTerm = lambda/m * [0; theta(2:end)]; 
...
grad = (1/m) * gradient + gradRegularizationTerm;
